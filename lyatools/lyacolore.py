...
def run_sbatch_script(box, colore_setup, lyacolore_setup, job, output_file, nnodes=8, job_dependency=None):
    ...
    colore_out_loc = Path(colore_setup.get('colore_dir')) / 'colore' / f'box-{box}' / 'results'
    lyacolore_out_loc = Path(lyacolore_setup.get('lyacolore_dir')) / 'lyacolore'/ f'skewers-{box}'
    conda_environment = job.get('env_lyacolore')
    config_file = os.path.join(__DIR__, "input_files/lyacolore/config_v9.0.ini")
    output_file=Path(lyacolore_setup.get('lyacolore_dir'))  / 'lyacolore' / f'skewers-{box}'/'scripts'/'lyacolore_script.sh'
    scripts_dir=Path(lyacolore_setup.get('lyacolore_dir'))  / 'lyacolore' / f'skewers-{box}'/'scripts'
...
def run_lyacolore(colore_setup, lyacolore_setup, job, dependency_ids=None):
    #Get box list
    ...
    for box in box_list:
        ...
        jobid = run_sbatch_script(box, colore_setup, lyacolore_setup, job, nnodes=8, output_file=f'lyacolore_script_{box}.sbatch', job_dependency=job_dependency)
        if no_submit is False:

...
Combines:
  ...
  • An ASCII multipole covariance matrix for the QSO auto-correlation
    (e.g. from a mock stack, or in the flat-matrix format used by RascalC).
...
Usage
-----
    python lyatools/scripts/build_global_cov3x2.py --help
    python lyatools/scripts/build_global_cov3x2.py \\
        --lya-cov   /path/to/lya-qso-cross-covariance.fits \\
        --qso-cov   /path/to/xi02_..._cov.txt \\
...
def read_lya_cov(path, scale=1.0):
    ...
    scale : float, optional
        Multiplicative factor applied to the covariance.  Use this to convert
        a covariance-of-the-mean (as stored in the analysis-stack
        ``full_cov.fits``, which equals the single-realization covariance
        divided by the number of stacked mocks N) into a single-realization
        covariance by passing ``scale = N``.  Default 1.0 (no change).
...
def read_multipole_cov(path, s_min, s_max, n_multipoles):
    """Read an ASCII multipole covariance matrix for the QSO auto block.

    The file stores the full covariance as a flat N×N matrix, one row per line,
    with data ordered as:
        [xi_0(s_1..s_n), xi_2(s_1..s_n), ..., xi_L(s_1..s_n)]
    where the s bins span exactly [s_min, s_max).  This layout is shared by
    mock-stack covariances and the RascalC multipole output format.
    ...
    if cov_full.shape[0] != cov_full.shape[1]:
        raise ValueError(
            f'Multipole covariance matrix is not square: {cov_full.shape}. '
    ...
    print(f'QSO auto multipole covariance: {n_cov}×{n_cov} ({n_s_cov} s-bins × '
...
def write_fits(cov, output_path, lya_cov_path, qso_cov_path,
               s_min, s_max, n_multipoles, lya_cov_scale=1.0):
    ...
    hdu.header['LYASCALE'] = (lya_cov_scale, 'Scale factor applied to LyA cov block')
    hdu.header['QSO_COV'] = (qso_cov_path, 'Source QSO auto multipole covariance file')
    ...
    hdu.header['COMMENT'] = 'Block-diagonal: LyA cross-cov + QSO auto multipole cov'
...
def main():
    ...
    parser.add_argument(
        '--lya-cov-scale', type=float, default=1.0,
        help='Multiplicative factor applied to the LyA covariance block. Set '
             'this to the number of stacked mocks N when --lya-cov is an '
             'analysis-stack full_cov.fits (covariance of the mean) but you are '
             'fitting a single realization, so the block becomes a '
             'single-realization covariance. Default: 1.0 (no scaling).')
    parser.add_argument(
        '--qso-cov', required=True,
        help='Path to the ASCII multipole covariance file for the QSO '
             'auto-correlation (flat n×n matrix; same layout as RascalC or '
             'a mock-stack covariance).')
    parser.add_argument(
    ...
    parser.add_argument(
        '--n-multipoles', type=int, default=2,
        help='Number of multipoles in the QSO auto measurement '
             '(e.g. 3 for ell=0,2,4). Default: 2 (ell=0,2)')
    args = parser.parse_args()

    # Read the LyA block
    lya_cov = read_lya_cov(args.lya_cov, scale=args.lya_cov_scale)

    # Read the QSO auto block
    qso_cov, s_centers = read_multipole_cov(
    ...
    write_fits(combined, args.output, args.lya_cov, args.qso_cov,
               ...
               lya_cov_scale=args.lya_cov_scale)

...
FINAL_DTYPE = np.dtype(
    [('NHI', 'f8'), ('Z', 'f8'), ('TARGETID', 'i8'), ('DLAID', 'i8'), ('SNR', 'f8')])


def make_dla_catalog(
    ...
    snr_cat_path = output_path / 'snr_cat.fits'
    snr_catalog = _read_snr_catalog(snr_cat_path, output_catalog['TARGETID'])
    output_catalog['SNR'] = snr_catalog['SNR_REDSIDE']

    output_file = output_path / 'hcd_truth_cat.fits'
    ...
    output_file = submit_utils.find_path(output_dir) / dla_cat_name

    if not output_file.is_file():
        print(
            f'Writing catalog for masking DLAs with NHI > {mask_nhi_cut:.2f},'
            f' SNR > {mask_snr_cut:.1f} and completeness {completeness:.2f}'
        )
        with fitsio.FITS(output_file, 'rw') as file:
...
def _read_snr_catalog(path, targetids):
    ...
    if not np.all(in_catalog):
        raise ValueError(
            'There are some TARGETIDs in the DLA catalog that are not in the SNR catalog.'
...
def _reduce_completeness(catalog, completeness):
    ...
    # Placeholder for a more complex completeness downsampling strategy
    # (e.g. function of SNR and NHI).
...
def main():
    ...
    parser.add_argument('--mask-snr-cut', type=float, default=2., required=False,
    ...
    make_dla_catalog(
        args.input_dir, args.output_dir, args.mask_nhi_cut, args.nhi_error_amplitude,
        args.mask_snr_cut, args.completeness, seed=args.seed, nproc=args.nproc
    )

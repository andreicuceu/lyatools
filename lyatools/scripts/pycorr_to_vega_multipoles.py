...
from the pycorr jackknife realizations when available, or from the diagonal of
an external multipole covariance matrix via --cov-file (e.g. from a mock stack
or a data covariance analysis).

# NOTE: This script requires the `pycorr` package and must be run within the
# "cosmodesi" conda environment, not the "vega" environment.

Usage
-----
    python lyatools/scripts/pycorr_to_vega_multipoles.py INPUT.npy OUTPUT.txt \\
        --ells 0,2,4 --smin 20 --smax 200 --ds 4 \\
        [--cov-file xi02_..._cov.txt]
"""
...
def main():
    ...
    parser.add_argument(
        '--cov-file', default=None,
        help='Optional ASCII multipole covariance (flat n x n matrix, one row per '
             'line). When given, the std columns are taken from sqrt(diag) of '
             'this matrix instead of the pycorr jackknife errors.')
    ...
    if isinstance(out, tuple) and len(out) >= 3:
        ...
        print('NOTE: jackknife errors not returned by pycorr; std columns '
              'default to 0 unless --cov-file is given.')
    ...
    # Optionally override std with sqrt(diag) of an external covariance matrix.
    if args.cov_file is not None:
        print(f'Reading covariance matrix {args.cov_file} for std columns.')
        cov = np.loadtxt(args.cov_file, comments='#')
        if cov.ndim != 2 or cov.shape[0] != cov.shape[1]:
            raise ValueError(
                f'Covariance matrix is not square: shape {cov.shape}.')
        n_cov = cov.shape[0]
        if n_cov % n_ells != 0:
            raise ValueError(
                f'Covariance matrix size {n_cov} is not divisible by the '
